...
function whoamiResponse(): Response {
  return {
    lines: [
      ...
      out("Global Leaders Award. 3.71 GPA."),
      out("Before grad school: multi-cloud MLOps at Jio Platforms."),
      out("Now: Cortex, Parallax, EEG research, tool-semantics."),
      divider,
      muted("reliable agents · context · evaluation · infrastructure"),
    ],
  };
}
...
function contactResponse(): Response {
  return {
    lines: [
      ...
      divider,
      muted("for technical discussions — email is the fastest path."),
    ],
    scrollTo: "contact",
  };
}
...
const FILTER_ALIASES: Record<string, ProjectFilter> = {
  ...
  mlops: { keywords: ["infrastructure", "pipeline", "ml"] },
  infra: { keywords: ["infrastructure", "kafka", "fastapi"] },
  infrastructure: { keywords: ["infrastructure", "kafka"] },
  agent: { tags: ["Agent Reliability", "MCP"], keywords: ["agent"] },
  agentic: { keywords: ["agent", "mcp"] },
  mcp: { tags: ["MCP"] },
  llm: { tags: ["LLM Evaluation"] },
  eval: { tags: ["LLM Evaluation", "Evaluation"], keywords: ["evaluation", "compatibility"] },
  evaluation: { tags: ["LLM Evaluation", "Evaluation"], keywords: ["evaluation"] },
  reliability: { tags: ["Agent Reliability"], keywords: ["reliability", "failure"] },
  memory: { keywords: ["memory", "mcp", "organizational"] },
  context: { keywords: ["memory", "context", "mcp"] },
  cortex: { keywords: ["cortex", "memory", "mcp"] },
  parallax: { keywords: ["parallax", "reliability", "judge"] },
  eeg: { keywords: ["eeg", "seizure", "chb"] },
  seizure: { keywords: ["eeg", "seizure"] },
  "tool-semantics": { keywords: ["tool", "mcp", "compatibility", "behavioral"] },
  tools: { keywords: ["tool", "mcp", "compatibility"] },
  semantics: { keywords: ["tool", "semantics", "compatibility"] },
  kafka: { tags: ["Kafka"] },
  neo4j: { tags: ["Neo4j"] },
  observability: { tags: ["OpenTelemetry"], keywords: ["opentelemetry", "jaeger"] },
  ai: { pillars: ["Research", "Engineering", "Product"] },
  ml: { pillars: ["Research", "Engineering"] },
  backend: { keywords: ["fastapi", "kafka", "infrastructure"] },
};
...
function sudoHireMeResponse(): Response {
  return {
    lines: [
      ...
      mono("  ✓ 100+ Airflow DAGs · 50M+ events/day · 45% lower MTTD"),
...
function matchNaturalLanguage(input: string): Response | null {
  ...
  if (/\b(can\s+you\s+)?(ship|deploy|production|productionize)\b/.test(q)) {
    return {
      lines: [
        accent("yes."),
        out("100+ Airflow DAGs · 50M+ events/day. Jenkins + Docker + K8s."),
        out("85% faster deploys. 45% lower MTTD across three clouds."),
